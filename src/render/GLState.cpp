    ...
    , _saveFrames(saveFrames)
{
    ...
    // createInstance builds the Music singleton only if the selected backend has a song
    // (createSelectedPlayer() returns non-null); otherwise instance() stays null and the demo
    // runs without music.
    Music::createInstance(saveFrames);
