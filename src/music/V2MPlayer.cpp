...
#include "V2MPlayer.h"
#include "EmbeddedSong.h" // generated by ojgl_add_demo(): resources::song (the embedded v2m data)
#include "Music.h"
...
{
    return resources::song ? ojstd::make_shared<V2MPlayer>(resources::song) : nullptr;
}

void V2MPlayer::play(Duration startTime)
{
    if (!_opened) {
        Init();
        Open(_song);
    ...
    sU32 ms = startTime.toMilliseconds<sU32>();
    Stop();
    dsClose();
    // Drop any stale events from a previous seek; the Play()+Tick() scan below repopulates them
    // and Music::_initSync() consumes them via popSyncEvents() after this returns.
    popSyncEvents();
    Play(ms);
    while (IsPlaying())
        Tick();
    Stop();
    dsInit(V2MPlayer::RenderProxy, this, GetForegroundWindow());
    Play(ms);
    // A seek restarts the DirectSound cursor from zero, so offset elapsedTime() by the seek target.
    _playOffset = startTime;
}
...
Duration V2MPlayer::elapsedTime() const
{
    long ms = ojstd::ftoi(dsGetCurSmp() * 1000.f / (44100.f * 4.f));
    return Duration::milliseconds(ms) + _playOffset;
}
