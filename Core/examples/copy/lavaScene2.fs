...
float timeScene0 = 8+5;
float timeScene1 = 16+5;
float timeScene2 = 40+5;
...
float lines(vec3 p, float time)
{
    ...
    float d1 = sdBox(p + vec3(0.3*sin(p.y * 2.3), 0, -0.1) +  0.8*noiseVec, vec3(0.2, 50.0 * k1, 0.48));
    //time *= 1.5;s
    float d2 = sdBox(p.yxz + vec3(0.3*sin(p.y * 2.3), 0, -0.1) +  0.8*noiseVec, vec3(0.2, 50.0 * k2, 0.48));
    float d3 = sdBox(p.xzy + vec3(0.3*sin(p.y * 2.3), 0, -0.1) +  0.8*noiseVec, vec3(0.02, 50.0 * k3, 0.5));
...
void main() {
    ...
    vec3 ro = vec3(0.3 - cos(time) + (k - k3)*0*sin(time*40)*0.5*cos(70*sin(cos(time))), (-8 - 5*k2) + (k - k3)*0*0.1*sin(30*time), (4.0 + k2 * 20));
    ...
    float fstr = smoothstep(60, 65, time);
    fragColor = vec4(color, 1.0);
    fragColor.rgb = (1 - fstr) * smoothstep(0, timeScene0, time) * fragColor.rgb / (fragColor.rgb + vec3(1.0));
}
