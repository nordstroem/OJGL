...
vec4 map(vec3 p, vec3 rd) 
{
	...
	vec4 box = vec4(dis, vec3(0.2, 0.5 * posSin(iTime), 0.5*posSin(iTime - 1)));
...
void main() {
	...
	//float k = 1 + 2*smoothstep(10.0, 10.5, time) * (1 - smoothstep(10.5, 11.0, time));
	float k = 1 + 1*posSin(5*cos(iTime)) * (1.0 - smoothstep(5.0, 10.0, time));
	u *= 16.0 / 9.0;
	u *= k;
	v *= k;
    vec3 ro = vec3(-2, -3, -15.0);
    vec3 tar = vec3(0.2*cos(10*iTime), 0.2*cos(10*iTime), 0.2*cos(10*iTime));
    ...
    for (int i = 0; i < 200; i++) {
		...
		if (d < 0.01) {
           // color = vec3(0.6, 0.2, 0.1);
            ...
            float spec =  pow(k, s);
            float str = 20.0/(0.01 + 0.4*dis + 0.03*dis*dis);
