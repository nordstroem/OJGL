...
vec4 tornado(vec3 p)
{
	p -= vec3(10*cos(2*sin(0.5*iTime)), -5, 10*sin(0.5*iTime + 2*cos(0.5*iTime)));
	float d2 = sdTorus(p.xzy - vec3(0, 0, 3.4 * sin(iTime)), vec2(2.2, 0.1)); 
	float l = 10;
	float bw = 2;
	float ph = inverseMix(p.y, -l/2, l/2);
	float noise = 0.03 * posSin(20.0 * p.y + 12 * iTime + p.x) + 0.05*posSin(2 * p.y + 5*iTime) + 0.03*posSin(5 * p.y + 5*iTime + 1);
	float buckle = 1 + posSin(3*p.y*sin(2*iTime) + p.x * cos(2*iTime));
	float d1 = sdCappedCylinder(p, vec2(bw * ph + 0.8*noise + 0.04*pow(buckle, 3.1), l/2));
	return vec4(d1, 0.1+1*ph, 0.1*ph, 0);
}
...
void main() {
	...
	float k = 1;// + 0.5*sin(5*cos(iTime));
	u *= 16.0 / 9.0;
	u *= k;
	v *= k;
    vec3 ro = vec3(-2, -6, -15.0);
    vec3 tar = vec3(0.2*cos(20*iTime), 0.2*cos(10*iTime), 0.2*cos(10*iTime));
    ...
    for (int i = 0; i < 200; i++) {
		...
		vec3 light = 60.0 * lightDisColor.yzw / (lol*lol);
		...
		if (d < 0.01) {
           // color = vec3(0.6, 0.2, 0.1);
            ...
            float dis = length(tornado(p).x - p);
