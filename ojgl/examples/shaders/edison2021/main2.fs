...
{
    DistanceInfo ed = edisonText(p);

    vec3 p2 = p;
    p2.z+=1.5;
    pMod2(p2.xy, vec2(0.217));
    p2.xz *= rot(sin(noise_2(10.1*p.xy) + iTime) * 3);
    DistanceInfo box = {sdBox(p2, vec3(1.2, 0.0, 0.01)), sphereType };

    DistanceInfo walls = { -sdRoundBox(p, vec3(3.0, 5.0, 5.0), 0.2), wallType };
  
    DistanceInfo res = un(box, ed);
    res = un(res, walls);
    return res;
}
...
{
    vec3 lightPosition = vec3(0, 1, -1.5);
    if (result.type != invalidType) {
        float d = length(lightPosition - result.position);
        float lightStrength = 0.0005 / (0.000001 + 0.001*d*d);
...
void main()
{
    ...
    rayOrigin = (iCameraMatrix * vec4(u, v, -0.7, 1.0)).xyz;
