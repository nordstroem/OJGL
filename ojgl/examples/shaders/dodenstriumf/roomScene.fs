...
vec3 distort(vec3 p) {
	...
	if (iTime > PART_TWIST) {
		...
		float l = length(p.xy);
		a += (p.z - PART_TWIST)*0.2*smoothstep(PART_TWIST + 5, PART_TWIST + 10, iTime);
		a -= (p.z - PART_TWIST)*0.2*smoothstep(PART_TWIST + 10, PART_TWIST + 15, iTime);
...
vec2 map(vec3 p, vec3 rd) 
{
	...
	if (iTime > PART_TWIST) {
		...
		float r = 0.1 + 0.3*(1.0 - smoothstep(0.0, 0.6, length(p.xy))) * (0.5 + 0.5 * sin(iTime* 5.0));
...
vec4 lightA(vec3 p, vec3 realp)
{
	...
	if (iTime*4.0 +  q *5.0 > 60 && iTime <= PART_TWIST) {

			col = vec3(0.0, 1.0, 0.2);
			strength = smoothstep(0 + 1, 0 + 2 + 1, iTime);

		
	}
	if (iTime > PART_END) {
		float m = (1.0 - (iTime - PART_END - 4.0));
		m = clamp(m, 0.0, 1.0);
		strength *= m;
	}
...
vec3 raymarch(vec3 ro, vec3 rd, vec3 eye) 
{
	...
	if (iTime < PART_TWIST) {
		fogAmount = mix(fogAmount, 0.5, smoothstep(70, 80, iTime*4.0 +  p.z));
	}
