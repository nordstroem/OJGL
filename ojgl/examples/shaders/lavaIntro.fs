...
vec2 fractalBox(in vec3 p)
{
   ...
   //p.z -= 2.0; 
    ...
    //mo(p.xz, vec2(sin(iTime), cos(iTime)));
...
vec2 map( in vec3 p )
{
    ...
    vec2 fs = fractalBox(   1./sc*(rotateAngle(direction, 0.*0.5*iTime*tf)*p - ballPos()));
    fs.x *= sc;
...
vec3 colorize(vec2 res, vec3 p, vec3 dir, int steps) 
{
    ...
    vec3 n = normal(p);
    float lf = min(2.5, 3.0 / (0.02 + 0.1*pow(length(p - lightPos), 3.0)));
    
    // Material properties
    float diffuse1 = 0.2*max(0.,dot(-light, n));
...
void main()
{
    ...
    vec3 ro = vec3(11.0, 10.+1.0*psin(iTime*0.) +DEBUG_W , 11.0);
